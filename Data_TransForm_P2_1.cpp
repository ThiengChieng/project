...
bool cmp2(const vector<string> &a, const vector<string> &b){
	if(a[0] != b[0]){
		return a[0] < b[0];
	}
	if(a[10] != b[10]){
...
int main(int argc, char const *argv[]){
	...
	for(int i = Position + 1; i < CutPath.size(); i++){
		FileName += CutPath[i];
	}
	fstream Doc(CutPath, ios::in);//開啟該檔案 
	stringstream ss;
	string TempContent;
	...
	if(FileContent.size() != 1){
		...
		for(it = FileContent.begin()+1; it < FileContent.end(); it++){
			if((*it)[24] < "09:00:00"){
	...
	sort(FileContent.begin()+1, FileContent.end(), cmp1);//排序看診資料 
	...
	for(int i = 0; i < FileContent.size(); i++){
		...
		stringstream WeekTrans, AgeTrans;
	...
	sort(FileContent.begin()+1, FileContent.end(), cmp2);//排序看診資料 
	 
	string name = FileName;
	string C_name = FileName;
	string OutPath = "Hos_Data\\Doc_Test\\Wa_Ch\\" + FileYear;
	string DirPath = "mkdir " + OutPath;
	system(DirPath.c_str());
	C_name.insert(4,"_Cluster");
	//更改檔案名稱
	 cout<<OutPath<<endl;
...
*/	
	//將清洗好的內容放於V01 
	cout<<FileName<<" Done"<<" Ave: "<<DocAverageTime<<" SD: "<<DocSD<<endl;
	//顯示已完成的檔案 
