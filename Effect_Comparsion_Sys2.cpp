...
int main(){
	string AnalysisFilePath = "";//要分析的檔案(我們)
	string VGHKSFilePath = "";//要分析的檔案(榮總)
	
	string FileYear = "";//檔案年份
	string FileName = "";//檔案名稱 
	
    fstream SourceTxt("javatoc.txt", ios::in);//開啟txt讀取檔案路徑 
	getline(SourceTxt, VGHKSFilePath);//獲得路徑
	getline(SourceTxt, AnalysisFilePath);//獲得路徑
	cout<<VGHKSFilePath<<" "<<AnalysisFilePath<<endl;
