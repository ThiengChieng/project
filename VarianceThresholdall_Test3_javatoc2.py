...
ctojava = open("ctojava2", mode='w')
writeCtoJava = 0
url = f.read()+'\\'
#url = 'D:/notepad++/JAVA_program/new2/Hos_Data/Doc_Train/Wa_Ch/2019/'#讀Wa_Ch
#url = 'D:/notepad++/JAVA_program/new/forall/Hos_Data/Doc_Train/Wa_Ch/2019/0059.csv'
...
for filepath in allFileList:
  #  print(url+filepath)
    ...
    dataframe = pd.read_csv(fileurl, encoding='mbcs')  # 解碼方式改為ansi解碼
    ...
    #dataframe = dataframe.drop(['看診日期', '看診科別', '看診診間', '病歷號', '醫師卡號', '跟診護理師卡號', '病患性別', '病患出生日期', '初複診', '掛號類別', '掛號序號', '班別早午診', '前次是否開立醫囑', '診斷碼1', '診斷碼2', '診斷碼3', '診斷碼4', '診斷碼5', '診斷碼6', '診斷碼7', '診斷碼8', '診斷碼9',
        ...
        if fileurl.endswith('.csv'):
            #print(fileurl)
            drop_list = ['看診日期', '看診科別', '看診診間', '病歷號', '醫師卡號', '跟診護理師卡號', '病患性別', '病患出生日期', '初複診', '掛號類別', '掛號序號', '班別早午診', '前次是否開立醫囑', '診斷碼1', '診斷碼2', '診斷碼3', '診斷碼4', '診斷碼5', '診斷碼6', '診斷碼7', '診斷碼8', '診斷碼9', '診斷碼10', '自助報到插卡時間', '醫師插卡時間', '醫師批價時間1', '醫師批價時間2', '醫師批價時間3', '醫師批價時間4', '醫師批價時間5', '醫師批價時間6', '醫師批價時間7', '醫師批價時間8', '醫師批價時間9', '預估看診時間', '診療時間']
            #print(drop_list)
            dataframe = dataframe.drop(columns=drop_list)
            Fts=pd.DataFrame()
            Fts['診療分群']=dataframe['診療分群']
            dataframe=dataframe.drop(['診療分群'],axis=1)
    ...
    # print(features3[1])
            ...
            label1 = ['看診星期','病患年齡','班別早午診_轉換','實際看診順序','掛號類別_轉換','病患性別_轉換','前次是否開立醫囑_轉換','初複診_轉換']
            for i in range(8):
        # print(features3[i])
                if features3[i]:
                    Fts[label1[i]] = dataframe[label1[i]]
                    label.append(i)
#    print(label)
    ...
    #print(Fts)
            ...
            year = fileurl[last2loc:lastloc]
            rloc = fileurl.rfind('Wa_Ch')
    ...
    # print(url[lastloc+1:lastloc+5])
      # print(docnumber)
            ...
            savepath = savepath+"Feature\\"+year+"\\All_"+result+"_FT"
        ...
        #print("hihih:"+savepath)
            Fts.to_csv(savepath,index = False, header=True)
            savepath=savepath[0:savepath.rfind('Doc_Train')]+'Doc_Test/Feature/2020'
